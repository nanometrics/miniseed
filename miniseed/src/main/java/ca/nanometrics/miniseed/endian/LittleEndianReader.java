...
public class LittleEndianReader implements EndianReader {
  ...
  public LittleEndianReader(byte[] buffer, int offset) {
    m_buffer = buffer;
    m_offset = offset;
  }
   ...
   */
  @Override
  public int read(byte[] bytes, int start, int bytesToRead) {
    System.arraycopy(m_buffer, m_offset, bytes, start, bytesToRead);
  ...
  @Override
  public int readInt() {
    int val = LittleEndian.get().readInt(m_buffer, m_offset);
    m_offset += INT_SIZE;
    return val;
  }
  ...
  public long readUInt() {
    long val = LittleEndian.get().readUInt(m_buffer, m_offset);
    m_offset += INT_SIZE;
    return val;
  }
  ...
  public short readShort() {
    short val = LittleEndian.get().readShort(m_buffer, m_offset);
    m_offset += SHORT_SIZE;
    return val;
  }
  ...
  public int readUShort() {
    int val = LittleEndian.get().readUShort(m_buffer, m_offset);
    m_offset += SHORT_SIZE;
    return val;
  }
  ...
  public long readLong() {
    long val = LittleEndian.get().readLong(m_buffer, m_offset);
    m_offset += LONG_SIZE;
    return val;
  }
  ...
  public int readInt24() {
    int val = LittleEndian.get().readInt24(m_buffer, m_offset);
    m_offset += INT24_SIZE;
    return val;
  }
  ...
  public float readFloat() {
    float val = LittleEndian.get().readFloat(m_buffer, m_offset);
    m_offset += FLOAT_SIZE;
    return val;
  }
  ...
  public double readDouble() {
    double val = LittleEndian.get().readDouble(m_buffer, m_offset);
